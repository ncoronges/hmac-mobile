...
{
    [super setUp];
    webServicePath = @"http://hmac-tests.appspot.com/resources/message";
...
- (void)testBasicMessageHash
{
    ...
    NSLog(@"Hash is: %@", hash);
    STAssertEqualObjects(hash,@"f7bc83f430538424b13298e6aa6fb143ef4d59a14946175997479dbc2d1a3cd8", @"hashes do not match");
}
...
- (void)testHashedMessageWithWebService 
{
    ...
    [request setURL:[NSURL URLWithString:[webServicePath stringByAppendingFormat:@"/form"]]];
    ...
    STAssertEqualObjects(success,@"1", @"failed with http form request");
    
}

- (void)testHashedJSONMessage 
{
    NSString *post = @"{\"message\":\"The quick brown fox jumps over the lazy dog\"}";
    NSString *key = @"key";
    NSString *hash = [MessageHasher hashMessage:post withKey:key];
    NSLog(@"got hash: %@",hash);
}
...
- (void)testSignHTTPRequest 
{
    ...
    NSString *hash = [MessageHasher hashMessage:newStr withKey:key];
    NSString *postLength = [NSString stringWithFormat:@"%d", [postData length]];
    NSMutableURLRequest *request = [[[NSMutableURLRequest alloc] init] autorelease];
    [request setURL:[NSURL URLWithString:[webServicePath stringByAppendingFormat:@"/post"]]];
    [request setHTTPMethod:@"POST"];
    [request setValue:postLength forHTTPHeaderField:@"Content-Length"];
    ...
    NSString* success = [object objectForKey:@"success"];
    NSLog(@"Success: %@", success); 
    STAssertEqualObjects(success, @"1", @"failed with http body request");
